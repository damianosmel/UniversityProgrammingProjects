...
PUBLIC void do_getmeminfo()
{
	...
	struct meminfo *pmstruct;
	
	/*aggregate size of holes.*/
	int sum_holelen=0;
	int temp;
	float sum=0.0;
	/*start from the head of the linked list.*/
	hp=hole_head;
	pmstruct->max_holesize=hp->h_len;
	pmstruct->min_holesize=hp->h_len; 
	pmstruct->hole_cnt=0;
	/*pass through all the holes to get useful information.*/	
	while( hp != NIL_HOLE )
	{	/*increase number of holes*/
		pmstruct->hole_cnt++;
		/*add hole size to aggregate size*/
		sum_holelen+=hp->h_len;
		/*find the max and min size up to this time*/
		if (pmstruct->max_holesize<hp->h_len) 
			{pmstruct->max_holesize=hp->h_len;}
		if (pmstruct->min_holesize>hp->h_len) 
			{pmstruct->min_holesize=hp->h_len;}
		/*go to the next hole in the list*/
		hp=hp->h_next;
		
	}
	sum=(float)( sum_holelen);	
	pmstruct->avg_size=(sum/pmstruct->hole_cnt);
	/*start again from the top of the list */
	hp=hole_head;
	sum=0.0;
	while( hp != NIL_HOLE )
	{
		/*(avg-hole size)^2 is added to sum.*/
		sum+=(pmstruct->avg_size-hp->h_len)*(pmstruct->avg_size-hp->h_len);
		/*go to the next hole. */
		hp=hp->h_next;
	}
	/* find the actual standard derivation */
	pmstruct->dev_size=sqrt(sum/8);
	/*copy the  "pm's" struct with the result 
	to the "user's" struct using sys_datacopy(...).   */
	sys_datacopy( PM_PROC_NR,(vir_bytes)pmstruct,
		     who_e,(vir_bytes) m_in.m1_p1,
		     (phys_bytes) sizeof( struct meminfo ) );
}
#if ENABLE_SWAP
/*===========================================================================*
 *				swap_on					     *
