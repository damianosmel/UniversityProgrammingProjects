...
//#include <time.h>
#include <sys/time.h>
//include openmp's library.
#ifdef _OPENMP
 #include <omp.h>
#endif
//solve a problem with freeing allocated memory
...
//pointers at start of the arrays of fact and diff are needed in order to each thread have its fact & its diff ,
// (like private variable in openmp).
//double *fact;
//double *diff;

//<==========================================others===============================================>
//flag to see if all part's temperature of the table have been successfully computed (diff[i]<EPSILON for each i).
int good_convertion=0;
//declare a variable to store the type of scheduling of threads
int schd_choice;
...
void RenderScene(void) {
	...
	//call jacobi in where "openmp" threads will help to heat computation.
	//see what user chose static or guided scheduling or heat computation, 
	//so call the correspond jacobi function.
	if( schd_choice == 1 )
		{
		//<=========================static schedule==============================>
		jacobi_st_schd();
	...
	// Show the graphics
	glutSwapBuffers();
	/*when openmp threads computed the heat distridution as long as benchmark mode is selected ,look the how many times heat source moved.if moved maxrepeats times print some useful informations for the experiment and close program.*/
	if( (benchmark == TRUE) && (moveSource() == maxrepeats) )
	{
		//at end inform the user for chosen settings such as number of threads,repeats 
		//and chosen scheduling type ( static or guided ) .
 		printf("End of computation. \n");
		printf("Informations :\n");
		printf("->Chosen number of pthreads: %d",num_thrd);
		if( schd_choice ==1 )
			printf("->Using Static scheduling of threads.\n");
		if( schd_choice ==2 )
			printf("->Using Guided scheduling of threads.\n");
		...
		//exiting without living waste ...free allocated memory for  diff[] and fact[].
		printf("Terminating experiment!\n");
		printf("Clearing used memory.\n");
		printf("Please Wait...\n");
		/*printf("Free allocated memory for  diff[] array.\n");	
		free(diff);
		printf("Free allocated memory for fact[] array.\n");
		free(fact);*/
		printf("Goodbye!\n");		
		exit(0);		
	}//close if for finish state. 
...
void jacobi_st_schd(){
	...
	int tid;
	//the following parallel sections will be computed by chosen by the user number of threads.
	omp_set_num_threads(num_thrd);
	//let all threads to initialize the correspond index of fact array.
	#pragma omp parallel private(i,j,k,tid)
	{
		//iterations , diff and fact are private variable owning to their declaration.
		...
		double diff;
	while(finish == FALSE){
			tid = (int) omp_get_thread_num();
			
			diff = 0.0;
			//having 'openmp' give work with static self scheduling to threads,
			//in the next to for loops ( compute ) and ( update ) heat arrays.
			//before computing the heat arrays....
			#pragma omp for  schedule(static) 
			for(i = 1; i <= cols; i++){
				for(j = 1; j <= rows; j++){
					for(k = 1; k <= faces; k++){
						after[i][j][k] = fact * (before[i - 1][j][k] + before[i + 1][j][k] + 
						before[i][j - 1][k] + before[i][j+ 1][k] + before[i][j][k-1] + before[i][j][k+1]);

						if(fabs(after[i][j][k] - before[i][j][k]) > diff)
						diff= fabs(after[i][j][k] - before[i][j][k]);
					}//close the innest if.
				}
			}//close extrenal if,so close the  omp for computing the heat distridution in the (x,y,z) axis. 
			//by default there is a barrier here
			good_convertion=0;
			//before updating the heat arrays.... 
			#pragma omp for schedule(static) 
			for(i = 1; i <= cols; i++){
				for(j = 1; j <= rows; j++){
					for(k = 1; k <= faces; k++){
			...
			}//close extrenal if,so close the omp for updating the heat distridution in the (x,y,z) axis. 
			#pragma omp atomic
			iterations++;
			//like in pthreads development if the diff corresponded to a thread of the parallel block 
			//is <=EPSILON increase the variable good_conversion.
			#pragma omp critical
			{
			if(diff <= EPSILON){
			...
			}//close critical section of adding to good_convertion. 
			//Synchronization point in order to have all threads check the previous if for good convertion of jacobi method 
			//before go on. 
			#pragma omp barrier
						
		
			//so here there only the master thread.
			//check if the good_convertion is equal to number of threads which participate in this parallel block
			//if it is equal u finish.
			if(good_convertion == num_thrd) 
			{	
				
					printf("In %d part:  Heat distribution stabilised after %d iterations (diff = %.24f)\n",tid, iterations, diff);
					//Synchronization point in order to have all threads print useful informations about heat distridution in their part, 
					//before go on. 
					#pragma omp barrier
					//heat computation finished for this source's move.				
					finish = TRUE;
					//let those conditions be checked in the RenderScene(),
					//in order to give the chance to  show the graphics.
					// by calling glutSwapBuffers() first.
					/*if(benchmark == TRUE){
...
}//close jacobi_st_schd .

//In this function jacobi developped having chosen guided self scheduling.
void jacobi_gd_schd(){
	...
	while(finish == FALSE){
			...
			tid = (int) omp_get_thread_num();
			diff = 0.0;
			//having 'openmp' give work with guided self scheduling to threads,
			//in the next to for loops ( compute ) and ( update ) heat arrays.
			//before computing the heat arrays....
			...
			}//close critical section for adding good_conversion.
			//Synchronization point in order to have all threads check the previous if for good convertion of jacobi method 
			//before go on. 
			#pragma omp barrier
...
int main(int argc, char** argv)
{
	...
	printf("Computing heat distridution with openmp's help.\n");
	printf("\nPlease give number of threads helping the computation\n");
	scanf("%d",&num_thrd);
	printf("\nThanks,You chose :\t%d\n",num_thrd);
	//allocate the memory needed to store diff and fact arrays.
	//if there is a problem print a warning message.
	/*if ( ( diff=(double *) malloc( num_thrd * sizeof (double) ) ) == NULL ||
	( fact=(double *) malloc( num_thrd * sizeof (double) ) ) == NULL )
	{printf("Needed memory could not be allocated.\n");}*/
	do{
		printf("Please,what type of scheduling for threads do you want ? \n");
		printf("Choices : \n");
		printf("1->static scheduling\t2->guided scheduling\n");
		...
		switch( schd_choice )
		{
			case 1:
				miss=0;
				//heat computation by threads will be done with static scheduling from openmp.
				//clear the time variable for static schedule computation.
				tm_cmp=0.0;
				printf("Please wait, computation with static scheduling from openmp starting. \n");
				break;
			case 2:
				miss=0;
				//heat computation by threads will be done with guided scheduling from openmp.
				//clear the time variable for guided schedule computation.
				tm_cmp=0.0;
				printf("Please wait, computation with guided scheduling from openmp starting. \n");
				break;
			case -1:
				miss=1;
				printf("Terminating experiment!\n");
	...
	//only if user chose correctly scheduling type proceed to glutMainLoop.
